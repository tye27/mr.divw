...
#' @param gen_cor If the exposure and outcome datasets are non-overlapping, then provide a K-by-K matrix for the estimated shared correlation matrix between the effect of the genetic variants on each exposure, where K is the number of exposure. The correlations can either be estimated, be assumed to be zero, or fixed at zero using non-overlapping samples of each exposure GWAS. Otherwise, provide a (K+1)-by-(K+1) correlation matrix where the last row and column corresponds to the SNP-outcome associations. Default input is NULL, meaning that an identity matrix is used as the correlation matrix.
